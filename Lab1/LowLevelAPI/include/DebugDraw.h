...
namespace Beta
{
	...
	public:
		...
		BE_API void EndLineList(Camera& camera, float zDepth = 0.0f);
		...
		//   zDepth = Depth of the object (when using a perspective camera).
		BE_API void AddCircle(const Vector2D& center, float radius, Camera& camera,
		...
		//   color   = The color of the circle. Defaults to white.
		//   zDepth = Depth of the object (when using a perspective camera).
		BE_API void AddRectangle(const Vector2D& center, const Vector2D& extents, Camera& camera,
			const Color& color = Colors::White, float zDepth = 0.0f);
